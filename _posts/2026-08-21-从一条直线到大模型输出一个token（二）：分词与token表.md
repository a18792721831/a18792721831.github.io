...
上一篇，我们把 230B 参数压缩成了一条过原点的直线，又通过加截距、升维、引入矩阵，搭好了数学地基。但留了个尾巴：$y = w \cdot x + b$ 这种公式，怎么处理"今天西安的天气怎么样？"这样一句话？
...
统计结果：$\text{freq}(\text{今},\text{天}) = 3$ 是唯一最高频对。合并！词表从 11 变成 12，多了一个新 token "今天"。

第二轮：语料更新为：

> 今天|天|气|好　今天|去|西|安　今天|怎|么|样　西|安|天|气|差

再次统计：$\text{freq}(\text{天},\text{气}) = 2$ 是唯一最高频对。合并！词表变成 13，多了"天气"。
...
![token表查表](https://picgo-1302191088.cos.ap-guangzhou.myqcloud.com/csdn/llm-series/fig26_lookup-v3.png)

**图2-8 token表 = 字符串和整数的一一映射表**

到这里，"今天西安的天气怎么样？"这 11 个字符，变成了一个长度为 6 的一维整数数组：

$$\mathbf{X} = [5237,\ 23872,\ 301,\ 16652,\ 19602,\ 1148]$$
