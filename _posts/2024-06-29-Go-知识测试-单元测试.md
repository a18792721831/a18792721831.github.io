...
layout: post
title: "Go-知识测试-单元测试"
date: 2024-06-29 13:23:35 +0800
categories: [golang, 单元测试, go, 单元测试原理, 单元测试执行过程]
description: "单元测试是指对软件中的最小可测试单元进行检查和验证，比如对一个函数的测试。单元测试要保证测试文件以_test.go结尾。测试方法必须以TestXxx开头。测试文件可以与源码处于同一目录，也可以处于单独的目录。_go 单元测试"
keywords: golang, 单元测试, go, 单元测试原理, 单元测试执行过程
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/140062867
> - 发布时间：2024-06-29 13:23:35
> - 阅读量：1
> - 分类：Golang专栏收录该内容, 订阅专栏
> - 标签：#golang, #单元测试, #go, #单元测试原理, #单元测试执行过程

## 摘要

文章浏览阅读1.4k次，点赞13次，收藏20次。单元测试是指对软件中的最小可测试单元进行检查和验证，比如对一个函数的测试。单元测试要保证测试文件以_test.go结尾。测试方法必须以TestXxx开头。测试文件可以与源码处于同一目录，也可以处于单独的目录。_go 单元测试
...
#### Go-知识测试-单元测试
  ...
  * [1\. 定义](<#1__1>)
  * [2\. 使用](<#2__6>)
  * [3\. testing.common 测试基础数据](<#3_testingcommon__17>)
  * [4\. testing.TB 接口](<#4_testingTB__67>)
  * [5\. 单元测试的原理](<#5__102>)
  *     * [5.1 context 单元测试的调度](<#51_context__112>)
    *       * [5.1.1 等待并发执行 testContext.waitParallel](<#511__testContextwaitParallel_138>)
      * [5.1.2 并发测试结束 testContext.release](<#512__testContextrelease_163>)
    * [5.2 测试执行 tRunner](<#52__tRunner_181>)
    * [5.3 启动测试 Run](<#53__Run_323>)
    * [5.4 启动并发测试 Parallel](<#54__Parallel_384>)
...
比如  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2a301ab64eab422378ce4d8662a7b8f8.png#pic_center)

执行单个TestXxx  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f55a81461dc749db6bf66a86c49556e5.png#pic_center)

执行整个测试文件  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e6644be4e65911ed824ece428e6f8a9a.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f005a116a29dc0f4b2dc0bde3f1c689b.png#pic_center)

然后会根据传入参数进行启动  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ca7ae897e299fc410e7138319a742713.png#pic_center)
