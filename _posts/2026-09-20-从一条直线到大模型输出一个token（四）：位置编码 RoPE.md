...
用 d=4 演示（两对维度，频率分别为 1 和 1/100），我们 6 个 token 的 PE 矩阵如图 4-3 下半部分。以位置 1 为例：$\sin(1) = 0.841$，$\cos(1) = 0.540$——图中每个数都能套公式手算验证。

#### 3.3 优缺点（带例子）

**优点举例**：训练时模型只见过 8192 个位置的 PE 矩阵，推理时来了 10000 个 token 的新闻稿——套公式 $\sin(10000/10000^{2i/d})$ 直接算出来就能用，不用像第二代那样担心"表里没有"。这正是第一代论文里 $\frac{1}{10000^{2i/d}}$ 设计成几何级数衰减频率的原因：**外推到任意长度都合理**。
...
至于"相对距离机制绕"——要靠三角恒等式 $\sin(m+n) = \sin m \cos n + \cos m \sin n$ 反推两个位置的差，公式推导上不算优雅。
...
$$\theta_i = 10000^{-2i/d}, \qquad i = 0, 1, \dots, 2047$$
...
用旋转矩阵写出来（$\mathbf{R}_m$ 表示位置 m 的旋转）：

$$\langle \mathbf{R}_m \mathbf{q},\ \mathbf{R}_n \mathbf{k} \rangle = \langle \mathbf{R}_{m-n} \mathbf{q},\ \mathbf{k} \rangle$$

左边是"各自按绝对位置旋转后点积"，右边是"只按相对位置 $(m-n)$ 旋转一次"。这个恒等式是三角函数和角公式 $\cos(m\theta)\cos(n\theta) + \sin(m\theta)\sin(n\theta) = \cos((m{-}n)\theta)$ 的矩阵版。

真实算一组数验证（演示向量 $[0.5, -0.3]$，$\theta=1$）：
...
3. 外推性：任意位置 m 都能算 $m\theta$，没有表的上限 ✓
...
- 频率 $\theta_1 = 10000^{0} = 1$（高维用高频），$\theta_2 = 10000^{-0.5} \approx 0.01$（低维用低频）
- 真实规模：d=4096 时拆 2048 对，频率从 $\theta_0=1$ 递减到 $\theta_{2047}=10000^{-2046/4096}=0.0001$

#### 6.2 为什么是 10000，为什么是递减

这两个问题合并回答：位置 m 转的角度是 $m\theta$，$\theta$ 决定"多远的位置才能转一圈"。

- $\theta=1$ 表示 $m$ 增加 $2\pi$（约 6.28）个位置就转一整圈；远距离的位置会"周期重合"——这正是高频维度负责"近距离区分"的原因
- $\theta=0.01$ 表示 $m$ 增加 $628$ 个位置才转一整圈；这就是低频维度，能区分"几公里外"的位置
...
第一对 $[-0.58, 0.91]$ 旋转 $1 \times 1 = 1$ rad：

$$x' = -0.58\cos 1 - 0.91\sin 1 = -1.079, \qquad y' = -0.58\sin 1 + 0.91\cos 1 = 0.004$$

第二对 $[0.27, -0.72]$ 旋转 $1 \times 0.01 = 0.01$ rad（几乎没转）：

$$x' = 0.27\cos 0.01 + 0.72\sin 0.01 = 0.277, \qquad y' = 0.27\sin 0.01 - 0.72\cos 0.01 = -0.717$$
...
$$\mathbf{X}_{今天} \cdot \mathbf{X}_{天气} = -1.5708$$
...
![有无RoPE对比](https://picgo-1302191088.cos.ap-guangzhou.myqcloud.com/csdn/llm-series/fig47_rope_vs_none-v3.png)
...
- 贯穿案例：6×4 矩阵逐行旋转，「西安」行完整手算 $[-0.58, 0.91] \to [-1.079, 0.004]$
