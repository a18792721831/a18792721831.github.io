...
### 目录结构
  ...
  1. [介绍](<#intro>)
  2. [Java语言的新特性](<#NewFeatureOfLanguage>)

2.1 [Lambdas表达式与Functional接口](<#lambdaAndFunctional>)

2.2 [接口的默认与静态方法](<#defaultAndStaticMethod>)

2.3 [方法引用](<#methodReferences>)

2.4 [重复注解](<#repeatingAnnotations>)

2.5 [更好的类型推测机制](<#betterTypeInference>)

2.6 [扩展注解的支持](<#extendedAnnotationsSupport>)

  3. [Java编译器的新特性](<#newFeatureOfCompiler>)

3.1 [参数名字](<#parameterNames>)

  4. [Java 类库的新特性](<#newFeatureOfLibraries>)

4.1 [Optional](<#optional>)

4.2 [Streams](<#streams>)

4.3 [Date/Time API (JSR 310)](<#datetimeAPI>)

4.4 [JavaScript引擎Nashorn](<#nashornJavaScript>)

4.5 [Base64](<#base64>)

4.6 [并行（parallel）数组](<#parallelArrays>)

4.7 [并发（Concurrency）](<#concurrency>)

  5. [新增的Java工具](<#newJavaTool>)

5.1 [Nashorn引擎: jjs](<#jjs>)

5.2 [类依赖分析器: jdeps](<#jdeps>)

  6. [Java虚拟机（JVM）的新特性](<#newFeatureOfJVM>)
...
需要记住的一件事是：[默认方法与静态方法](<#defaultAndStaticMethod>)并不影响函数式接口的契约，可以任意使用：
...
Stream API、[Lambda表达式](<#lambdaAndFunctional>)与[方法引用](<#methodReferences>)在[接口默认方法与静态方法](<#defaultAndStaticMethod>)的配合下是Java 8对现代软件开发范式的回应。更多详情请参考[官方文档](<http://docs.oracle.com/javase/tutorial/collections/streams/index.html>)。
...
我们在后面的[Java新工具](<#newJavaTool>)章节会再次谈到Nashorn。
