...
date: 2024-06-01 18:26:13 +0800
categories: [golang, go, go 并发管理, Mutex, 互斥锁, 独占锁]
description: "互斥锁是并发程序中对共享资源进行访问控制的主要手段，Go 语言提供了非常简单易用的 Mutex。Mutex 是结构体类型，对外暴露了 Lock 和 Unlock 两个方法，用于加锁和解锁。自旋对应于CPU的PAUSE指令，CPU对该指令什么都不做，相当于CPU空转，对程序而言相当于sleep了一小段时间，时间非常短，当前实现是 30个时钟周期。自旋过程中会持续探测 Locked 是否变为0 ，连续两次探测奸恶就是在执行这些 PAUSE 指令，不同于sleep，不需要将协程转为睡眠状态。_go mutex"
keywords: golang, go, go 并发管理, Mutex, 互斥锁, 独占锁
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/139377732
> - 发布时间：2024-06-01 18:26:13
> - 阅读量：1
> - 分类：Golang专栏收录该内容, 订阅专栏
> - 标签：#golang, #go, #go 并发管理, #Mutex, #互斥锁, #独占锁

## 摘要

文章浏览阅读1.3k次，点赞34次，收藏12次。互斥锁是并发程序中对共享资源进行访问控制的主要手段，Go 语言提供了非常简单易用的 Mutex。Mutex 是结构体类型，对外暴露了 Lock 和 Unlock 两个方法，用于加锁和解锁。自旋对应于CPU的PAUSE指令，CPU对该指令什么都不做，相当于CPU空转，对程序而言相当于sleep了一小段时间，时间非常短，当前实现是 30个时钟周期。自旋过程中会持续探测 Locked 是否变为0 ，连续两次探测奸恶就是在执行这些 PAUSE 指令，不同于sleep，不需要将协程转为睡眠状态。_go mutex
...
#### Go-知识并发控制mutex
  ...
  * [1\. 介绍](<#1__3>)
  * [2\. 数据结构](<#2__6>)
  *     * [2.1 Mutex 结构体](<#21_Mutex__7>)
    * [2.2 Mutex 方法](<#22_Mutex__25>)
  * [3\. 加锁解锁过程](<#3__31>)
  *     * [3.1 简单加锁](<#31__32>)
    * [3.2 加锁被阻塞](<#32__164>)
    * [3.3 简单解锁](<#33__169>)
  ...
  *     * [4.1 什么是自旋](<#41__227>)
    ...
    * [4.3 自旋的优势](<#43__239>)
    * [4.4 自旋的问题](<#44__241>)
  * [5\. Mutex 模式](<#5_Mutex__244>)
  *     * [5.1 Normal 模式](<#51_Normal__246>)
    * [5.2 Starving 模式(饥饿模式)](<#52_Starving__249>)
  * [6\. Woken 状态](<#6_Woken__253>)
  * [7\. 重复解锁引发 panic](<#7__panic_257>)
  * [8\. 总结](<#8__262>)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/53e210cad96de031cbd01343d9fb3b07.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f96f3fca1367cff2f3b9606c8b9df024.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/35feee1e7202bf2749972382948ff28f.png#pic_center)
...
假设加锁时，锁已经被其他协程占用了：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0997b86c50a8268e42ed3490225f992b.png#pic_center)
...
假设解锁时，没有其他协程阻塞：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5f721d71dc4f7b99a6fcd3dc19f216b6.png#pic_center)
...
假设解锁时，有一个或多个协程阻塞：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e8d0e12f0840c01e471eeb31b879bd3f.png#pic_center)
