...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f18ebc062c98bc394b6bb694ef360cad.png)  
感觉有些抽象，稍微调整下布局：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8090072961deb0327742c328a678779f.png)  
这是一个巨复杂的类。  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b8f69923d6ad5bcb56ceee25dffcc62e.png)

## 2\. ReentrantLock 实现Lock接口

既然ReentrantLock实现了Lock接口，那么，我们就从Lock入手。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6ca967b47d2d22d6b59b287f4476a3ca.png)

### 2.1 tryLock

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2c985dbe004c0c489d192e7bbc1f41e1.png)  
可以看到，ReentrantLock的tryLock方法只是内部Sync的nonfairTryAcquire的代理，请从目录跳转到3.2.  
这里传入的1表示每次重入，锁的重入层数加1.

### 2.2 tryLock(long,TimeUnit)

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5aefaefec7930de6dee3e0dfcbe5c225.png)  
这个方法也是做了代理。代理的目标方法是AQS的方法：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5696470b01b0a8541198acb4821bf1bf.png)  
tryAcquireNanos的方法的时序图如下：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f6286ac3f1534f99b76f8d6663d3f47f.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/450f3ab7f2326405047d0c4864ded4a3.png)  
ReentrantLock中的lock方法代理指向了ReentrantLock内部实现的Sync类的lock方法，请跳转到3.1.

### 2.4 lockInterruptibly

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/322fa3da818abf44ad11a5b3313374f3.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f688a66c8246c044fecd2b5f115acdc6.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/eb5a83a286436e558b7af8ab4fb7aeae.png)  
newCondition代理指向ReentrantLock内部类的newCondition方法，请跳转3.5.

### 2.7 其他方法

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0ce00c36ac294e34a809ed547ea0996a.png)

## 3\. ReentrantLock 内部Sync实现了AQS

这是ReentrantLock内部Sync的结构：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/db0efb85144711872482f61cba7e1a55.png)

### 3.1 lock

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/dbbd77da8b144acb76ece264ec9f5bb9.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3610025139d41df27a5d05d0781799c6.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d890cf6c60bd2226ff7a4c99f27f4f89.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/72af95f32c62d0963eb2c53f9b1e0306.png)  
将当前线程与锁持有线程进行比较，如果相等，则返回true，表示当前线程已经获取到了锁，否则返回false，表示当前线程没有获取到锁。

### 3.5 newCondition

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5942848d2d8f17eb21ff46e04363c978.png)  
...
这个方法获取线程节点的线程的方法。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b23066bb74fd63a00128a9a1ec0199e4.png)  
这个方法返回线程节点存储的线程。  
线程节点的状态等于0表示锁空闲，那么就表示线程节点对应的线程已经被清空了(头结点)。

### 3.7 getHoldCount

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/64d5cf059443f38fc40f92cb7a64158e.png)  
如果是独占模式，那么返回当前线程持有锁的层数。

### 3.8 isLocked

查询此锁是否由任意线程保持。此方法用于监视系统状态，不用于同步控制。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/48468eaa4b23fd2658dd624d64af26de.png)

### 3.9 readObject

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/53f69d742baad8a912e7a9c892b240db.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8b507200bb8ef4c4e7bb8a24316b47e2.png)  
FairSync的lock方法，使用的是AQS的acquire方法。  
AQS的acquire方法请看  
[Java基础–AQS原理](<https://blog.csdn.net/a18792721831/article/details/106730738>)  
中的第5.6.2小节  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d42fa42ed0defa87aee00cc6f910e896.png)  
lock的公平性是由FairSync的tryAcquire方法实现的。

### 4.2 FairSync–tryAcquire

这是tryAcquire的时序图  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/83c9e364872dd1f2fe07545442e1dce1.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1aaa58f115867d8536cc7254b5213164.png)  
...
![这是不公平尝试获取锁的时序图](https://i-blog.csdnimg.cn/blog_migrate/73e756db8f048df4139f777f17cd980c.png)  
不公平尝试获取锁的方法直接代理使用ReentrantLock内部Sync的nonfairTryAcquire方法。  
详细请见3.2.  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f14d2fcbe00bf26fd6fa078919f16d67.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b44898ac1f9b58ea6778b3ab6874e841.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/62fca4513e672db07491029f56a28f47.png)  
使用无参数的构造方法，调用的是不公平重入锁。

### 5.2 ReentrantLock(boolean)

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/01df756f3a75ebca9afb15d8ec365dbb.png)  
