...
layout: post
title: "Go-知识string"
date: 2024-03-05 16:47:56 +0800
categories: [golang, go, string, go sdk, go源码]
description: "在Go语言的设计中， string 通常指的是字符串的字面量，而且字符串一般情况下存储的位置也是只读，而不是堆或者栈(这里可能一定程度上借鉴了汇编之类的语言)。所以设计为string不可修改。当string不可修改后，带来的好处是string变得非常轻量，而且可以放心的在go-routine中进行传递，不用考虑并发冲突和内存拷贝。_go string 双引号转单引号"
keywords: golang, go, string, go sdk, go源码
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/136483336
> - 发布时间：2024-03-05 16:47:56
> - 阅读量：1
> - 分类：Golang专栏收录该内容, 订阅专栏
> - 标签：#golang, #go, #string, #go sdk, #go源码

## 摘要

文章浏览阅读1.3k次，点赞31次，收藏18次。在Go语言的设计中， string 通常指的是字符串的字面量，而且字符串一般情况下存储的位置也是只读，而不是堆或者栈(这里可能一定程度上借鉴了汇编之类的语言)。所以设计为string不可修改。当string不可修改后，带来的好处是string变得非常轻量，而且可以放心的在go-routine中进行传递，不用考虑并发冲突和内存拷贝。_go string 双引号转单引号

---

#### Go-知识string

  * [1\. string 的用法](<#1_string__3>)
  *     * [1.1 string 的声明](<#11_string__4>)
    * [1.2 string 双引号和反单引号的区别](<#12_string__14>)
    * [1.3 string 拼接](<#13_string__36>)
    * [1.4 string 和 []byte 互转](<#14_string__byte__42>)
  * [2\. string 的特性](<#2_string__61>)
  *     * [2.1 UTF编码](<#21_UTF_62>)
    * [2.2 string 值不可修改](<#22_string__77>)
  * [3\. 标准库函数](<#3__82>)
  * [4\. string 的实现原理](<#4_string__104>)
  *     * [4.1 数据结构](<#41__110>)
    * [4.2 编码](<#42__126>)
    * [4.3 string 拼接](<#43_string__130>)
    * [4.4 []byte 转 string](<#44_byte__string_255>)
    * [4.5 string 转 []byte](<#45_string__byte_311>)
    * [4.6 []byte 转为 string 的编译优化](<#46_byte__string__355>)
  * [5\. 总结](<#5__363>)
  *     * [5.1 为什么Go里面string不允许修改](<#51_Gostring_364>)
    * [5.2 什么时候使用string,什么时候使用[]byte](<#52_stringbyte_368>)
    * [5.3 string 获取长度](<#53_string__379>)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9d23f97bc565c8ec4de26aac6b037b08.png#pic_center)
...
比如：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f9115f18ffbc9ccde6b49ab1c98f5757.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/08be5f9f7072268e4b2b8e7014307d65.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a8d1512e8e7eead693aa77e06456af2e.png#pic_center)
...
因为`""`是空string，在统计中会忽略。  
再看看 `tmpBuf`是个啥：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9cab0fb19b990cb5e9881b6149644d3d.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cf1cce96efdd883723dc9f7309bb139f.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cfdba2d156f3d28ee359b01bea41593d.png#pic_center)