---
layout: post
title: "【转载】RabbitMQ消息可靠性分析"
date: 2019-07-02 09:03:34 +0800
categories: [RabbitMQ消息可靠性, RabbitMQ如何保证消息安全, RabbitMQ如何确保消息发布安全, RabbitMQ如何确保消息消费安全, RabbitMQ的整体安全性]
description: "深入探讨RabbitMQ消息传递的四大阶段，包括生产者到交换器、交换器到队列、队列存储和消费者确认，分析消息可靠性提升策略，如事务机制、发送方确认、持久化、镜像队列及消息确认机制。"
keywords: RabbitMQ消息可靠性, RabbitMQ如何保证消息安全, RabbitMQ如何确保消息发布安全, RabbitMQ如何确保消息消费安全, RabbitMQ的整体安全性
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/94429203
> - 发布时间：2019-07-02 09:03:34
> - 阅读量：322
> - 分类：RabbitMQ专栏收录该内容, 订阅专栏
> - 标签：#RabbitMQ消息可靠性, #RabbitMQ如何保证消息安全, #RabbitMQ如何确保消息发布安全, #RabbitMQ如何确保消息消费安全, #RabbitMQ的整体安全性

## 摘要

文章浏览阅读322次。深入探讨RabbitMQ消息传递的四大阶段，包括生产者到交换器、交换器到队列、队列存储和消费者确认，分析消息可靠性提升策略，如事务机制、发送方确认、持久化、镜像队列及消息确认机制。

---

转自：<https://juejin.im/post/5d196fc56fb9a07ed4411825>

[](</user/5d11b8816fb9a07f0870a360>)
...
![image](https://user-gold-cdn.xitu.io/2019/7/1/16bab5cba8841e30?imageView2/0/w/1280/h/960/format/webp/ignore-error/1) 如图所示，从AMQP协议层面上来说： 
...
![image](https://user-gold-cdn.xitu.io/2019/7/1/16bab5cba19c4b5e?imageView2/0/w/1280/h/960/format/webp/ignore-error/1) 事务机制在一条消息发送之后会使发送端阻塞，以等待RabbitMQ的回应，之后才能继续发送下一条消息。相比之下，发送方确认机制最大的好处在于它是异步的，一旦发布一条消息，生产者应用程序就可以在等信道返回确认的同时继续发送下一条消息，当消息最终得到确认之后，生产者应用便可以通过回调方法来处理该确认消息，如果RabbitMQ因为自身内部错误导致消息丢失，就会发送一条nack（Basic.Nack）命令，生产者应用程序同样可以在回调方法中处理该nack命令。 
...
![image](https://user-gold-cdn.xitu.io/2019/7/1/16bab5cba490e4cc?imageView2/0/w/1280/h/960/format/webp/ignore-error/1) 备份交换器其实和普通的交换器没有太大的区别，为了方便使用，建议设置为fanout类型，如若读者想设置为direct或者topic的类型也没有什么不妥。需要注意的是消息被重新发送到备份交换器时的路由键和从生产者发出的路由键是一样的。备份交换器的实质就是原有交换器的一个“备胎”，所有无法正确路由的消息都发往这个备份交换器中，可以为所有的交换器设置同一个AE，不过这里需要提前确保的是AE已经正确的绑定了队列，最好类型也是fanout的。如果备份交换器和mandatory参数一起使用，那么mandatory参数无效。 
