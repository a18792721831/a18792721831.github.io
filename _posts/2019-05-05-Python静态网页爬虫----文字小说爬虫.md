...
#### 文字爬虫
  ...
  * [1.爬虫的行为](<#1_8>)
  * [2.项目设计](<#2_11>)
  * [3.获取静态网页的html代码](<#3html_44>)
  * [4.获取下一章的url](<#4url_181>)
  * [5.获取小说的章节名字](<#5_225>)
  * [6.获取小说正文](<#6_239>)
  * [7.保存章节小说](<#7_266>)
  * [8.合并临时文件](<#8_282>)
...
首先看下目标网页：  
这个就是本次测试的小说书籍：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a96b7c9a2afc76570da1b780b4be6d11.png)  
这是正文部分：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2912f8e1352f1de164d0f24ce097289e.png)  
...
安装requests库也是非常的简单：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/61c4a20e223ff908d879bd43198f0a38.png)  
或者更简单的直接import requests，然后requests会报错，在idea的ide下，使用alt+回车，选择下载requests库，让ide去下载第三方库。  
第三方库安装后再python的sdk的安装目录的下面：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c70c3683d39591c5576c0ca4264adf7b.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c343a30458a921d07fccc8ec7fade635.png)  
所以如果发现找不到第三方库  
在project中看下工作空间的sdk  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/78fcce82df6966ad9d4ff5c38518c4c0.png)  
然后看下sdk的package信息  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/24b3c81b4c2426c53c42e8ac72417403.png)  
如果发现没有第三方库，有时需要等待一段时间，或者重启，让ide扫描package信息。

言归正传，如何测试获取静态网页的html方法是否正确呢？  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/24d431665d20207456361d63e5e22799.png)  
在方法的下面直接进行测试：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/91fbd9d5d6a5024c61a85aedb1f87757.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a8208ee56cdfe97c65bdb77c846b4d0a.png)  
然后切换到  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5f1c9c2e2589fbc5cc18e52f4228ab8a.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/207c4341ff5be142195f21b1f01cc9cd.png)  
但是这个地址是相对地址，是相对网站的地址，并不是绝对地址，不能直接访问：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6e3834e0a67cb1aae161d7c33702ec26.png)  
网站的地址可以直接在代码中定义，也可以通过logo获取。  
至此，获取下一个网页的url的所有的信息都找到了，那么如何在html中找到呢？  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d88e495778d79a6d159b1aa28622f21f.png)  
网站的地址比较简单，直接在class为header_logo的div标签的a标签内。  
div内也只有一个a标签。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0e6059ffc75996e1112b17a14214b381.png)  
...
pip install bs4  
下载完成后增加如下代码进行测试：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8bc74d7ebfdc3d8ac4faf1b4dc05b6a6.png)  
按住ctrl+shift+f10直接运行python![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/7d85017e2488cc3c9781f29178ca5d97.png)  
会发现有异常，这是因为我们没有指定解析的方式，但是这个类库非常的强大，及时没有指定解析的模式，也能成功的解析，但是为了不输出异常信息，所以还是指定一下解析方式（具体请参考官方文档(有中文语言的文档)）  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1fe4e0ba0931239c7e50475c5e1480b8.png)  
我们拷贝这个地址到浏览器验证  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1962fa848b9bd9650a2a04a0a9988b1b.png)  
完全正确。
...
所以，小说的章节也需要正确的获取。  
采用同样的方式进行获取章节名字：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0774e418b0bff7569eead33c4ceebdcd.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cbd0bbe19665524ad8a5d6844adeffa9.png)

## 6.获取小说正文

获取小说正文就最简单了  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/97a1889fb90304c8bbc5be011acba8ff.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9ce2718ea12f5a8a63a8f90447dfdf70.png)

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/debb885d5c611bd027c5fd3be83647e6.png)  
这里有一个比较坑爹的地方：  
调试结果如上：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/260c9ae5c1ec0941a23974e04c7c1af7.png)  
实际输出结果
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b37d77b9ea9d6d90b05a7e7a68540f76.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e882073e7765d3ab5734671cfe939b2e.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f4ff32745941a6cb8c49f635d8767db3.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/280dcaa8257848dcdbff38ac55bfcd4d.png)  
然后为了友好的进行交互：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3fed54b8a02a08b3cad7e05e6160261f.png)  
这里采用了全局队列的处理方式：  
首先把初始连接url加入全局的url队列中：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/38d5cb5090a62e0ae29e0125ae955e04.png)  
创建临时文件夹  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a016919a225c6553146880cc0efad678.png)  
输出进度信息  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/7bb9580a8a40113bc9ef12f09b8434aa.png)  
然后临时文件也需要一个全局队列  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/04e649d5508a914f6689b4a1167ef69a.png)  
把临时文件目录放入缓冲队列。  
临时文件名字生成：  
当前网页的url的哈希值  
当所有的章节都下载完成后，合并临时文件（合并临时文件后会删除临时文件）  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/198da20aff3d5e6a296c5a8b9bfe5144.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/99256f33237129d14ae1ed1929f0ce87.png)  
合并完所有的临时文件后会删除临时文件夹。  
这个项目因为最后一章是回去不到下一章的链接的，所以需要用try——except处理。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a8931a8cedae7ff68829474cc3bd79dc.png)  
整个项目需要输入三个参数：  
