...
date: 2024-07-01 10:44:02 +0800
categories: [golang, go, go 测试, go Main测试]
description: "声明一个，名字比较特殊的测试，参数类型为test.M指针。如果声明了一个这样的测试，那么当前测试程序不是直接执行各项测试，而是将测试交给TestMain调度。_go testmain"
keywords: golang, go, go 测试, go Main测试
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/140094681
> - 发布时间：2024-07-01 10:44:02
> - 阅读量：1
> - 分类：Golang专栏收录该内容, 订阅专栏
> - 标签：#golang, #go, #go 测试, #go Main测试

## 摘要

文章浏览阅读1k次，点赞18次，收藏9次。声明一个，名字比较特殊的测试，参数类型为test.M指针。如果声明了一个这样的测试，那么当前测试程序不是直接执行各项测试，而是将测试交给TestMain调度。_go testmain
...
使用`go test -v`执行  
![在这里插入图片描述](https://picgo-1302191088.cos.ap-guangzhou.myqcloud.com/csdn/csdnimg/b18ffc7bcbcb470add5bafc7d2ed90eb.png)
