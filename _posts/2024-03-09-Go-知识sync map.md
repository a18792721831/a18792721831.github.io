...
layout: post
title: "Go-知识sync map"
date: 2024-03-09 18:14:29 +0800
categories: [golang, go, sync map, sync map源码, 并发安全map]
description: "sync map 将互斥锁内置实现并发读写，将互斥锁的范围仅仅限定在dirty表，减少锁等待和锁调用，提升性能。因为dirty表和read表总是在争取保持一致，所以大部分读场景下，read表就能查询到数据，适合读多写少。sync map 中read表和dirty表都会持有key，所以内存占用上会比较大，而且在写多读少的场景下，因为既要遍历read表，又要遍历dirty表，性能上会比较慢。_go sync.map"
keywords: golang, go, sync map, sync map源码, 并发安全map
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/136588343
> - 发布时间：2024-03-09 18:14:29
> - 阅读量：2
> - 分类：Golang专栏收录该内容, 订阅专栏
> - 标签：#golang, #go, #sync map, #sync map源码, #并发安全map

## 摘要

文章浏览阅读2k次，点赞26次，收藏18次。sync map 将互斥锁内置实现并发读写，将互斥锁的范围仅仅限定在dirty表，减少锁等待和锁调用，提升性能。因为dirty表和read表总是在争取保持一致，所以大部分读场景下，read表就能查询到数据，适合读多写少。sync map 中read表和dirty表都会持有key，所以内存占用上会比较大，而且在写多读少的场景下，因为既要遍历read表，又要遍历dirty表，性能上会比较慢。_go sync.map
...
#### Go-知识sync map
  ...
  *     * [1.1 声明](<#11__10>)
    * [1.2 增删改查](<#12__15>)
    * [1.3 增强操作](<#13__58>)
  * [2\. sync map 使用注意](<#2_sync_map__84>)
  *     * [2.1 多读少写](<#21__87>)
    * [2.2 类型安全风险](<#22__91>)
    * [2.3 不能拷贝和传递](<#23__109>)
  * [3\. 实现原理](<#3__113>)
  *     * [3.1 数据结构](<#31__114>)
    * [3.2 read表数据结构](<#32_read_132>)
    * [3.3 entry 的数据结构](<#33_entry__144>)
    * [3.4 sync map 的结构图](<#34_sync_map__153>)
    * [3.5 插入数据](<#35__157>)
    * [3.6 查找数据](<#36__206>)
    * [3.7 再次插入](<#37__257>)
    * [3.8 删除数据](<#38__286>)
