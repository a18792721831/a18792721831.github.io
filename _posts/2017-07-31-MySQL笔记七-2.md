...
layout: post
title: "MySQL笔记七-2"
date: 2017-07-31 14:16:47 +0800
categories: [mysql]
description: "mysql的查询、子查询及连接查询一、mysql查询的五种子句    where(条件查询)、having（筛选）、group by（分组）、order by（排序）、limit（限制结果数）    1、where常用运算符：        比较运算符            > ,  < ,=  , != （< >）,>=   ,   <=              in(v1,v2..vn)_mysql笔记7-2"
keywords: mysql
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/76435673
> - 发布时间：2017-07-31 14:16:47
> - 阅读量：399
...
文章浏览阅读399次。mysql的查询、子查询及连接查询一、mysql查询的五种子句    where(条件查询)、having（筛选）、group by（分组）、order by（排序）、limit（限制结果数）    1、where常用运算符：        比较运算符            > ,  < ,=  , != （< >）,>=   ,   <=              in(v1,v2..vn)_mysql笔记7-2
