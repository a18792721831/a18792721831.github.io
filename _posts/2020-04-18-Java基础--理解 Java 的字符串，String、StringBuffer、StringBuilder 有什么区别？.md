---
layout: post
title: "Java基础--理解 Java 的字符串，String、StringBuffer、StringBuilder 有什么区别？"
date: 2020-04-18 14:17:05 +0800
categories: [Java字符串, StringBuilder, StringBuffer, String.intern, 字符串常量池]
description: "本文深入探讨Java中的String、StringBuffer和StringBuilder的区别，包括它们的设计原理、实现细节和性能考量。文章还介绍了字符串缓存机制、字符串排重以及Java9中字符串的紧凑化改进。"
keywords: Java字符串, StringBuilder, StringBuffer, String.intern, 字符串常量池
---

> **文章信息**
> - 原文链接：https://jiayq.blog.csdn.net/article/details/105595691
> - 发布时间：2020-04-18 14:17:05
> - 阅读量：788
> - 分类：java核心专栏收录该内容, 订阅专栏
> - 标签：#Java字符串, #StringBuilder, #StringBuffer, #String.intern, #字符串常量池

## 摘要

文章浏览阅读788次。本文深入探讨Java中的String、StringBuffer和StringBuilder的区别，包括它们的设计原理、实现细节和性能考量。文章还介绍了字符串缓存机制、字符串排重以及Java9中字符串的紧凑化改进。
...
有人粗略统计过，把常见应用进行堆转储（Dump Heap），然后分析对象组成，会发现平均 25% 的对象是字符串，并且其中约半数是重复的。如果能避免创建重复字符串，可以有效降低内存消耗和对象创建开销。  
...
但实际情况估计会让你大跌眼镜。一般使用 Java 6 这种历史版本，并不推荐大量使用 intern，为什么呢？魔鬼存在于细节中，被缓存的字符串是存在所谓 **PermGen** 里的，也就是臭名昭著的 **“永久代”** ，这个空间是很有限的，也基本不会被 FullGC 之外的垃圾收集照顾到。所以，如果**使用不当** ，**OOM** 就会光顾。  
...
前面说到的几个方面，只是 Java 底层对字符串各种优化的一角，在运行时，字符串的一些基础操作会直接利用 JVM 内部的 Intrinsic 机制，往往运行的就是特殊优化的本地代码，而根本就不是 Java 代码生成的字节码。Intrinsic 可以简单理解为，是一种利用 native 方式 hard-coded 的逻辑，算是一种特别的内联，很多优化还是需要直接使用特定的 CPU 指令。当然，你也可以在启动实验应用时，使用下面参数，了解 intrinsic 发生的状态。
...
仔细观察过 Java 的字符串，在历史版本中，它是使用 char 数组来存数据的，这样非常直接。但是 Java 中的 char 是两个 bytes 大小，拉丁语系语言的字符，根本就不需要太宽的 char，这样无区别的实现就造成了一定的浪费。密度是编程语言平台永恒的话题，因为归根结底绝大部分任务是要来操作数据的。
...
基于jdk8
    ...
    String two = new String("one");//1.堆空间创建对象
    String one = new String("one");//1.堆空间创建对象
    System.out.println(one == two);//堆空间中的不同对象
    //false
    ...
    String one = "one";//1.字符串常量池
    System.out.println(two.intern() == two);//字符串常量池引用 ！= 堆空间引用
    //false
    ...
    System.out.println(one.intern() == one);//常量池与常量池比较
    //true
    System.out.println(one == two);//常量池与堆空间比较
    //false
    System.out.println(one.intern() == two);//常量池与堆空间比较
    //false
    System.out.println(one == three);//常量池与堆空间比较
    //false
    System.out.println(one.intern() == three);//常量池与堆空间比较
    //false
    System.out.println(two.intern() == two);//常量池与堆空间比较
    //false
    System.out.println(two == three);//堆空间与堆空间比较
    //false
    System.out.println(two.intern() == three);//常量池与堆空间比较
    //false
    System.out.println(three.intern() == three);//常量池与堆空间比较
    //false
    System.out.println(one.intern() == two.intern());//常量池与常量池比较
    //true
    System.out.println(one.intern() == three.intern());//常量池与常量池比较
    //true
    System.out.println(two.intern() == three.intern());//常量池与常量池比较
    //true
    ...
    String one = "onetwo";
    //=> 直接取字符串常量池 "onetwo"
    System.out.println(two.intern() == two);//1.字符串常量池已有，2.字符串常量池与堆空间比较
    //false
    System.out.println(two.intern() == one);//1.常量池已有；2.字符串常量池与字符串常量池
    //true
    System.out.println(one == two);//字符串常量池与堆空间
    //false
    ...
    String two = new String("one") + new String("two");
    //堆空间创建："one","two","onetwo"
    String one = "onetwo";//将"onetwo"放入字符串常量池
